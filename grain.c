...
int main(int argc, char **argv) {
	...
	int key1[10] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
		IV1[8] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
	    ks[10],
	    plaintext2[10]={0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10},
	...
	keystream_bytes(&mygrain,ks,10);
	mygrain2 = mygrain;
	encrypt_bytes(&mygrain,plaintext2,encrypted_text,10);
	decrypt_bytes(&mygrain2,encrypted_text,decrypted_text,10);
	...
	keystream_bytes(&mygrain,ks,10);
	mygrain2 = mygrain;
	encrypt_bytes(&mygrain,plaintext3,encrypted_text,10);
	decrypt_bytes(&mygrain2,encrypted_text,decrypted_text,10);
	printData(key1,IV1,ks, plaintext3, encrypted_text, decrypted_text, 10);
